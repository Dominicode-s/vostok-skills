...
### v2.5.7
- **Fixed: Pack Mule overweight sound looping on the inventory tab** (community report). `Interface.UpdateStats` was double-counting the Pack Mule bonus: base game already adds `gameData.xpCarry * 2.0` using the level we sync every frame, and our override was adding `level * cfg_carry_per_level` on top. With the default `cfg_carry_per_level = 2.0` that silently gave players ~4 kg/level instead of the advertised 2 kg/level — and, worse, made `super()` see a lower capacity than we did. When a player's weight landed between base's view and ours, base would call `character.Overweight(true)` (playing the SFX), our override would call `character.Overweight(false)`, and the next `UpdateStats` tick (fired on every inventory action) would repeat — SFX loop. Fix: only add the *delta* from the configured value, i.e. `level * (cfg_carry_per_level - 2.0) + prestige`, so base and the override now agree on the capacity, and `character.Overweight(active)` is called at most once per real transition.
- **Side effect:** players on v2.5–v2.5.6 have silently been getting ~4 kg/level of carry capacity instead of 2 kg. v2.5.7 restores the advertised value. Raise Pack Mule's per-level value in MCM if you want the v2.5–v2.5.6 behaviour back.
...
- `Temperature` was already on the clean scaled-delta pattern (no mutation), unchanged.
- `Health` keeps its scoped `gameData.xpRegen = 0` across super — that's a narrow one-field bypass for a specific base-game block we explicitly want to skip, not general state injection. Limited risk; leaving as-is.
